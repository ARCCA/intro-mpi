...
#SBATCH -p compute
#SBATCH -o mpiio.independent.out.%j
...
WDPATH=/scratch/$USER/mpi_training/mpiio.independent.$SLURM_JOBID
mkdir -p $WDPATH
cd $WDPATH

# Copy the python code to the run directory
cp $SLURM_SUBMIT_DIR/mpiio.independent.py .

# Run a number of copies of the code equal to the number of
# MPI processes requested.
mpirun -np 4 python3 mpiio.independent.py
